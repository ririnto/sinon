...
- Plugin agents also support `disallowedTools`, `effort`, `maxTurns`, `skills`, `memory`, `background`, and `isolation: worktree` when those fields change required runtime behavior.
...
6. Add only supported plugin-agent fields; omit `model` unless the agent needs a fixed model and keep examples in the body.
