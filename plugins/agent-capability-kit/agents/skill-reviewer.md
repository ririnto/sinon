...
- Recognize `license`, `compatibility`, `metadata`, and experimental `allowed-tools` as portable optional fields.
- Enforce Sinon house style separately: Sinon-published skills use only `name` and `description` unless the task and repository policy permit an optional portable field.
- Require the display title to be the first H1 heading, not a frontmatter field.
- Require version, baseline, source, owner, and official-documentation notes to live in the body when ordinary use needs them.
...
1. Critical: portable frontmatter schema violations, missing ordinary path, mandatory support-file dependency, or misleading trigger.
