...
const numberField = (message: JsonObject, key: string): number => {
  const value = message[key];
...
const hoverResult = (uri: string, position: JsonObject): JsonObject => {
...
const handleRequest = (message: JsonObject): JsonObject | undefined => {
