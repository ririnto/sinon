...
3. Use WebSocket only when the server needs persistent bidirectional delivery or server-initiated pushes.
...
Claude Code reconnects HTTP servers with bounded exponential backoff for eligible transient failures.
