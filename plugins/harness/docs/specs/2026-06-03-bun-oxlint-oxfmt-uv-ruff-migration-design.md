...
- The custom-rule path requires Node on the target machine: oxlint loads JS plugins via a Node runtime, not Bun (spike-verified Q6). This is a new prerequisite beyond the standalone-Bun model. Built-in oxlint rules and oxfmt run from the binary and need no Node, so even without Node the bun `check` keeps built-in coverage plus the 19 structural rules; only the 2 custom-plugin conventions go dark. On plugin-load failure oxlint emits a non-silent error (`Failed to parse oxlint configuration file.` then `x Failed to load JS plugin: <path>` with the verbatim Node error); the adapter MUST surface this and fail the custom-rule capability rather than passing silently.
