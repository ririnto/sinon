...
- oxlint owns all 14 TS AST code-style rules.
  ...
  - Custom JS-plugin rules WITH fixers for the bespoke conventions that have no built-in: `earlyReturn`, `implicitLambdaIt`, `importOverFqn`, `multilineDocStyle`, `publicDeclarationDocComment`, `greaterThanComparison`, `leafFunctionBlankLines`. The alpha status of the JS-plugin API is accepted (user decision, 2026-06-03), mirroring the gradle/ktlint full detection+fix integration.
- oxfmt owns formatting.
- The bun harness engine retains only the 19 structural rules. The 14 TS AST rule modules are removed once oxlint covers them.

### Components

- `.oxlintrc.json` (config): `categories`, built-in rule severities, `jsPlugins` pointing to custom rule files, and `overrides` as needed.
- Custom oxlint rule plugins: ESLint v9-compatible JS modules using `create(context)` returning AST visitors and reporting via `context.report({ node, message, fix })`. Exact plugin module shape and config keys MUST be verified against official oxc docs at implementation.
...
3. The adapter maps oxlint diagnostics into `Finding[]`, normalizing severity and recording fix safety from oxlint's fix metadata (`safe` for `--fix`, `unsafe` for `--fix-suggestions`/`--fix-dangerously`).
...
2. Formatting-oriented harness rules that oxfmt subsumes (evaluate `leafFunctionBlankLines` specifically; it may be more specific than oxfmt's blank-line handling and stay a lint rule) are reconciled to avoid conflicting edits.
...
- Exact built-in rule-ID mapping, the oxlint plugin module signature, and oxfmt config keys are implementation details to be verified against official oxc/oxfmt docs, not design gates.
...
## Open Verification Items (Resolve at Implementation)

- Exact oxlint built-in rule IDs that cover each mappable convention.
- The oxlint `jsPlugins` config schema and the custom-rule module/`context.report({fix})` signature.
- oxfmt config key names and defaults, and whether `leafFunctionBlankLines` is subsumed by oxfmt.
- ruff rule-family selection that maps to existing uv conventions.
