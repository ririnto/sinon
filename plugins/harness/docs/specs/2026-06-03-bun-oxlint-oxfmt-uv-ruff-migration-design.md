...
- oxlint owns 9 of the TS AST code-style rules: 6 built-in + 3 custom. B1 removals eliminated `earlyReturn`, `silentCatch`, `emptyCatchBlock` (as a harness rule), and `importOverFqn` from the bun manifest.
  ...
  - Custom JS-plugin rules WITH fixers for the remaining bespoke conventions: `publicDeclarationDocComment` (WARN), `multilineDocStyle` (WARN). The alpha status of the JS-plugin API is accepted (user decision, 2026-06-03), mirroring the gradle/ktlint full detection+fix integration. After B1 removals and user-requested `greaterThanComparison` removal, no custom rules for `earlyReturn`, `silentCatch`, `emptyCatchBlock`, `importOverFqn`, or `greaterThanComparison` remain on the bun stack.
...
- The bun harness engine retains only the 19 structural rules. TS AST code-style detection is fully migrated to oxlint (6 built-in + 2 custom, 8 total).

### Components

- `.oxlintrc.json` (config) at `.oxlintrc.json` (bun stack root, not runtime subdir): built-in rule severities (slash form, all set to `error` so oxlint always emits — the harness re-derives the authoritative severity from the manifest), `jsPlugins: ["./docs/harness/bun/oxlint-plugins/harness.mjs"]`, and `overrides` as needed. Spike-verified (Q3): `jsPlugins` paths resolve relative to the config file location. With the config at the project root, the plugin path resolves from there. The adapter relies on oxlint's auto-discovery of `.oxlintrc.json` from cwd and does NOT use an explicit `--config` flag, symmetric with `.oxfmtrc.json`.
- Custom oxlint rule plugin: a single ESM module `docs/harness/bun/oxlint-plugins/harness.mjs` (default export `{ meta: { name: "harness" }, rules: { "<rule-id>": { meta: { fixable: "code" }, create(context) { ... } } } }`) declaring the 2 custom rules (`multilineDocStyle`, `publicDeclarationDocComment`) and reporting via `context.report({ node, message, fix })`. Spike verification (oxlint 1.68.0): a `.mjs` ESM plugin loads and autofixes WITHOUT any `package.json` (Q1), so the existing standalone-Bun self-install model is preserved — no `package.json`, no declared devDependencies. Provisioning is by `bunx oxlint@1.68.0` / `bunx oxfmt@0.53.0`, which self-provision the pinned binaries with no separate install step (Q4, Q5). `bunx oxlint` loads the JS plugin on Bun's own JavaScript runtime — no separate Node runtime is required (Q6, empirically confirmed: the custom rules still fire with `node` absent from PATH), preserving the standalone-Bun model.
...
A rule-by-rule equivalence assessment determined that `wildcardImport` → ruff `F403` is the only safe migration; ruff owns it as a single-owner category (`select = ["F403"]`). The remaining six libcst code-style rules stay on libcst as the policy-correct end-state, for two distinct reasons:

- No ruff equivalent exists: `leadingUnderscore` (ruff pep8-naming respects PEP 8's `_` weak-private convention rather than forbidding it), `multilineDocStyle` (no rule enforces block-only docstrings), and `tripleQuoteInlineComment` (no tokenizer rule for comments sharing a triple-quote line).
...
- The custom-rule path needs no separate Node runtime: `bunx oxlint` loads the JS plugin on Bun's own JavaScript runtime (spike-verified Q6, re-confirmed empirically — with `node` absent from PATH the custom rules still fire), so the standalone-Bun model is preserved with no new prerequisite. Built-in oxlint rules and oxfmt run from the binary; the custom-plugin conventions run through the same `bunx oxlint` invocation. On plugin-load failure oxlint emits a non-silent error (`Failed to parse oxlint configuration file.` then `x Failed to load JS plugin: <path>` with the verbatim runtime error); the adapter MUST surface this and fail the custom-rule capability rather than passing silently.
- oxlint `--format json` carries no per-diagnostic fix metadata (spike-verified); fix safety is therefore a static per-rule policy in the adapter rather than read from oxlint. Fix application is NOT auto-run by `harness-format` (which owns formatting via `oxfmt --write` only); `oxlint --fix` stays an explicit developer command. This is behavior-preserving against the current `harness-format` contract, which auto-applies only `FORMAT_ALLOWLIST` categories and leaves safe lint fixes reported-but-unapplied.
...
- oxlint `jsPlugins` config schema and the custom-rule module/`context.report({fix})` signature: confirmed working under Bun-invoked oxlint (plugin loaded on Bun's JS runtime, fixer rewrote source).
...
- Plugin runtime + load-failure signal (Q6): `bunx oxlint` loads JS plugins on Bun's own runtime (no separate `node` required — empirically confirmed with `node` absent from PATH) and emits a clear `Failed to load JS plugin: <path>` error on load failure.
