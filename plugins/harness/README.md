...
- Provide workflow templates with bounded concurrency, handoff states, workspace policies, prompt contracts, and proof-of-work expectations so agents can participate in task-level orchestration without requiring a daemon or task poller.
...
| uv/Python | ruff (`F403` via ruff CLI) for wildcard-import detection; LibCST for the remaining 11 AST rules | Python code-style checks for the selected uv slice via ruff and LibCST. |
| Bun/TypeScript | oxlint (Oxc parser) for code-style; harness engine for structural, manifest, and filesystem rules | oxlint enforces 8 TypeScript code-style rules (5 built-in: no-console, curly, no-underscore-dangle, no-namespace, ban-ts-comment; 3 custom oxlint JS-plugin rules: greaterThanComparison, multilineDocStyle, publicDeclarationDocComment) and oxfmt normalizes layout (indentation and blank-line collapsing). The harness engine retains 19 structural, manifest, and filesystem rules for the selected Bun slice. |
...
- This plugin prepares repository knowledge, guardrails, templates, and validation paths. It does not run background services or manage task queues.
