...
- Keep external orchestration facts out of the core product surface: this plugin can prepare repository knowledge, guardrails, and evidence paths, but it does not run a daemon, launch a service client, or manage per-task workspaces.
...
- Keep external orchestration daemon behavior, hosted task tracking, per-task workspace scheduling, service protocol handling, and runtime-stack provisioning outside this plugin. Document those as separate target-specific orchestration integrations when needed.
...
- External daemon implementation, hosted task polling, service clients, or per-task workspace orchestration
