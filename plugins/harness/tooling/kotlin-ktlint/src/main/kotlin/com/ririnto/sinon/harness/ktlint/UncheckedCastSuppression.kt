...
class UncheckedCastSuppression :
    Rule(
        ruleId = RuleId("harness:unchecked-cast-suppression"),
        about = About(),
        usesEditorConfigProperties = setOf(FORBIDDEN_SUPPRESSIONS, ALLOWED_SUPPRESSIONS)
    ),
    RuleAutocorrectApproveHandler {
    companion object {
        val FORBIDDEN_SUPPRESSIONS: EditorConfigProperty<String> =
        ...
        val ALLOWED_SUPPRESSIONS: EditorConfigProperty<String> =
            EditorConfigProperty(
                type =
                    PropertyType(
    ...
    private class SuppressAnnotationVisitor(
        ...
        private val castRelatedTokens: Set<String> =
            setOf(
                "UNCHECKED_CAST",
                "USELESS_CAST",
                "CAST_NEVER_SUCCEEDS",
        ...
        override fun visitAnnotationEntry(annotation: KtAnnotationEntry) {
            ...
            if (calleeText == Suppress::class.simpleName || calleeText == "kotlin.${Suppress::class.simpleName}") {
                annotation.suppressedTokens().firstOrNull { token -> token in forbiddenTokens }?.let { detectedToken ->
                    emit(
                ...
                .flatMap { expression ->
                    when (expression) {
                        is KtStringTemplateExpression -> {
                            listOfNotNull(expression.extractStringValue())
                        }
                        ...
                        is KtCollectionLiteralExpression -> {
                            expression.children
                                ...
                                .mapNotNull { template -> template.extractStringValue() }
                        }
