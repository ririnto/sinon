...
assert_common_assets_structure() {
  ...
  require_text "$common_assets_root/WORKFLOW.github.md" "Git-Contained Review"
  require_text "$common_assets_root/WORKFLOW.github.md" "change description"
  require_text "$common_assets_root/WORKFLOW.gitlab.md" "Git-Contained Review"
  require_text "$common_assets_root/WORKFLOW.gitlab.md" "change description"
  if grep -Eq 'gh (issue|pr)|Closes #' "$common_assets_root/WORKFLOW.github.md"; then
    printf '%s\n' '[common assets] GitHub workflow must use Git-contained records' >&2
    exit 1
  fi
  if grep -Eq 'glab (api|mr)|Closes #' "$common_assets_root/WORKFLOW.gitlab.md"; then
    printf '%s\n' '[common assets] GitLab workflow must use Git-contained records' >&2
    exit 1
  fi
