...
| Trigger | {task, release, scheduled check, or manual request} |
...
Progression inspired by repository-local workflow patterns. Each step gates the next; do not skip validation or review.
...
3. Compare with the selected integration base and record the sync result.

### Step 2: Implement

1. Make the smallest coherent change across docs, code, and checks.
2. Update the workpad or plan checklist as work progresses.
3. Do not expand scope; record out-of-scope improvements as separate follow-up tasks.
