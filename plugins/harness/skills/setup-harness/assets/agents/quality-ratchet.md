...
description: >-
  Advance one readiness or known-violation ratchet. Use this agent when guardrails move from visibility to advisory, shared gates, or error-new-debt mode.
model: sonnet
---
...
Advance one readiness or violation ratchet. Do not install the harness or implement product changes.
...
1. Read current stage, target stage, and known violations.
2. Identify the smallest safe ratchet step.
3. Update readiness or known-violation records only for that step.
4. Confirm no undocumented new violation is introduced.
