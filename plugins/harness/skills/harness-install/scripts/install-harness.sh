...
copy_one_git_hook() {
    ...
    cp "$src" "$tmp" || {
        rm -f "$tmp"
        rmdir "$tmp_dir"
        error "[copy_git_hook] failed to copy generated hook: $src"
    }
    chmod +x "$tmp" || {
        rm -f "$tmp"
        rmdir "$tmp_dir"
        error "[copy_git_hook] failed to mark hook executable: $tmp"
    }
    mv "$tmp" "$dst" || {
        rm -f "$tmp"
        rmdir "$tmp_dir"
        error "[copy_git_hook] failed to install git hook: $dst"
    }
    rmdir "$tmp_dir"
