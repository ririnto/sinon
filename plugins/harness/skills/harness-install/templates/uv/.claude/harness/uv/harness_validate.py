...
REQUIRED_DOC_HEADINGS = ["## Purpose", "## When To Update", "## Required Evidence", "## Validation Link"]
REQUIRED_AUTHORED_DOCS = [path for path in REQUIRED_FILES if path.startswith("docs/") and path.endswith(".md")]
LEAK_PATTERNS = [
...
def compare_manifest_list(failures: list[str], manifest: dict[str, object], key: str, expected: list[str]) -> None:
...
def validate() -> list[str]:
    ...
    compare_manifest_list(failures, manifest, "requiredDirectories", REQUIRED_DIRECTORIES)
    compare_manifest_list(failures, manifest, "emptyDirectoryKeepFiles", EMPTY_DIRECTORY_KEEP_FILES)
    ...
    generated_text = "\n".join([agents_text, claude_text, read_text(ROOT / "ARCHITECTURE.md")])
    evolution_text = "\n".join([agents_text, claude_text, read_text(ROOT / ".claude/harness/evolution-log.md")])
    ...
    if "docs/generated/db-schema.md" not in generated_text:
        failures.append("repository docs must state that docs/generated/db-schema.md is only an example, not a required scaffold file")
    if "source command" not in generated_text or "regeneration trigger" not in generated_text:
        failures.append("repository docs must describe generated-artifact source command and regeneration trigger metadata")
    if "discovery" not in evolution_text or "maintenance" not in evolution_text:
        failures.append("repository docs must state that the harness may evolve across development phases")
...
def validate_keep_files(failures: list[str]) -> None:
    for keep in EMPTY_DIRECTORY_KEEP_FILES:
        ...
        if not real_files and not is_safe_file(keep_path, failures):
            failures.append(f"empty directory must keep placeholder or real files: {relative(directory)}")
...
def validate_agents(failures: list[str]) -> None:
    ...
    files = sorted(path for path in safe_walk(directory, failures) if path.parent == directory and path.suffix == ".md")
    if not files:
...
def validate_skills(failures: list[str]) -> None:
    ...
    files = sorted(path for path in safe_walk(directory, failures) if path.name == "SKILL.md")
    if not files:
...
def validate_templates(failures: list[str]) -> None:
    for group in TEMPLATE_GROUPS:
        if not is_safe_directory(ROOT / f".claude/harness/templates/{group}", failures):
            failures.append(f"missing template group: .claude/harness/templates/{group}")


def validate_active_assets(failures: list[str]) -> None:
    excluded = ROOT / ".claude/harness/templates"
    active_roots = [ROOT / "AGENTS.md", ROOT / "CLAUDE.md", ROOT / "ARCHITECTURE.md", ROOT / "docs", ROOT / ".claude/agents", ROOT / ".claude/skills", ROOT / ".claude/harness", ROOT / ".github"]
    for base in active_roots:
        paths = safe_file_or_walk(base, failures)
        for path in paths:
            if not path.is_file() or excluded in path.parents or path.suffix not in {".md", ".txt", ".json", ".yml", ".yaml"}:
                continue
            text = read_text(path)
    ...
    for base in [ROOT / ".claude/harness", ROOT / ".claude/skills"]:
        ...
        for path in safe_walk(base, failures):
            ...
            if line.startswith("#!") and not line.startswith("#!/usr/bin/env "):
                failures.append(f"executable script should use /usr/bin/env shebang: {relative(path)}")
