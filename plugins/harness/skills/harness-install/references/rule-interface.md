...
- Validator: Maven command generated by the installer uses `git ls-files` to build escaped, repo-root-anchored `spotlessFiles`, runs `mvn validate`, then runs Spotless `check` on Git-tracked Java files (Palantir Java formatter + import order + unused import removal + trailing-whitespace hygiene).
...
- Check command: generated Maven Spotless command with `git ls-files` and `spotlessFiles`
- Format command: use the generated Maven `git ls-files` wrapper shape with `spotless:apply` and the same escaped, repo-root-anchored `spotlessFiles` value.

### uv

- Validator: `uv run scripts/check.py` (thin wrapper invoking `uvx --with "ruff>=0.15.15,<0.16.0" ruff check` on Git-tracked Python files)
...
- Format command: `uv run scripts/format.py` (thin wrapper invoking `uvx --with "ruff>=0.15.15,<0.16.0" ruff format` on Git-tracked Python files)
...
- Format command: `bun run format` (-> `sh scripts/format.sh` -> `bun install --no-save` -> `bunx ultracite fix --` on Git-tracked JavaScript and TypeScript files).
