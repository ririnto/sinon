                ...
                return {
                    ...
                    ReturnStatement(node) {
                        ...
                        if (body && body.type === "BlockStatement") {
                            ...
                            const isLast = statements.length > 0 && statements[statements.length - 1] === node;
                            if (!isLast) {
                                const name = current.id ? current.id.name : "<anonymous>";
                                context.report({
                                    node,
                                    message: `function \`${name}\` has an early return; restructure with single exit`,
                                });
                            }
                        }
                    },
            ...
            create(context) {
                return {
                    CatchClause(node) {
                        ...
                        let safe = false;
                        const inspect = (child) => {
                            if (!safe && (child.type === "ThrowStatement" || isLoggingCall(child))) {
                                safe = true;
                            }
                        };
                        for (const statement of statements) {
                            inspect(statement);
                            walkChildren(statement, inspect);
                        }
                        if (!safe) {
                            context.report({
                                node,
            ...
            create(context) {
                ...
                return {
                    ...
                    MemberExpression(node) {
                        ...
                        if (parent && parent.type === "MemberExpression" && parent.computed === false && parent.object === node) {
                            return;
                        }
                        let count = 0;
                        let current = node;
                        ...
                        if (count >= 2 && current.type === "Identifier" && !importedNames.has(current.name)) {
                            const chain = context.sourceCode.getText(node);
                            context.report({
                                node,
                                message: `fully qualified name \`${chain}\` used inline; add an import and use the simple name`,
                            });
                        }
                    },
                };
