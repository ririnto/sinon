...
/**
 * Renders a collection of validation findings in biome+ruff hybrid diagnostic format.
...
export function renderFindings(
  ...
  const safeFixCount = findings.filter(
    (f) => f.fix && f.fix.safety === "safe",
  ).length;
  findings.forEach((finding) => {
    ...
    if (!hasFile) {
      lines.push(
        `[${severityLabel}] ${finding.category}: ${finding.message}`,
      );
    } else if (!hasLocation) {
      lines.push(
        `${finding.file} [${severityLabel}] ${finding.category}: ${finding.message}`,
      );
    } else {
      lines.push(
        `${finding.file}:${finding.startLine}:${finding.startColumn ?? 1} [${severityLabel}] ${finding.category}: ${finding.message}`,
      );
      const snippetLines = getSnippet(
        root,
        finding.file,
        finding.startLine,
      );
    ...
    if (finding.fix) {
      ...
      if (finding.fix.edits && finding.fix.edits.length > 0) {
        ...
        lines.push("");
        lines.push("   Before:");
        extractEditText(root, firstEdit).split("\n").forEach((line) => {
          if (line) {
            lines.push(`   - ${line}`);
          }
        });
...
function getSnippet(
  ...
  return [
    ...
    ...(after !== null
      ? [
          `   ${String(after).padStart(lineNumWidth, " ")} │ ${fileLines[after - 1] ?? ""}`,
        ]
      : []),
  ];
}
...
function extractEditText(
  ...
  return [
    ...
    ...Array.from(
      { length: edit.endLine - edit.startLine - 1 },
      (_, idx) => lines[edit.startLine + idx],
    ),
