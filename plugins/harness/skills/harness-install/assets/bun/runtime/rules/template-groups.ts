 ...
 */
export const templateGroupsRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "templateGroups",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.templateGroups;
	...
	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.templateGroups).parameters,
		);
		const targetRoot =
			typeof parameters.targetRoot === "string" ? parameters.targetRoot : "";
		...
		return groups.flatMap((group) => {
			...
			return ctx.isDirectory(path)
				...
				: [
						{
							severity: ctx.severityOf(manifest, "templateGroups"),
							category: "templateGroups",
