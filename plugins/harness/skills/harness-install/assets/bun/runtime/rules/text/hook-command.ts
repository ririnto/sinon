...
export const hookCommandRule: HarnessCheckRule = {
    ...
    validate(ctx: RuleContext): readonly Finding[] {
        ...
        const validationCommandLine = lines.findIndex((line) => line.startsWith("# Harness validation command: "));
        const validationCommand =
            validationCommandLine >= 0
                ? (lines[validationCommandLine]?.replace("# Harness validation command: ", "").trim() ?? "")
                : "";
