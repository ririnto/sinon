...
export const leadingUnderscoreRule: HarnessCheckRule = {
    ...
    applies(ctx: RuleContext): boolean {
        return ctx.isEnabled("leadingUnderscore") && ctx.stackSources("leadingUnderscore").length > 0;
    },
...
function readRuleConfig(ctx: RuleContext): RuleConfig {
    const parameters = ctx.readJsonObject(ctx.categoryObject("leadingUnderscore").parameters);
    const allowedNames = new Set(["_"].concat(ctx.readStringArray(parameters.allowedNames)));
    const allowedPatterns = ctx.readStringArray(parameters.allowedPatterns).map((pattern) => new RegExp(pattern));
...
function validateBasename(ctx: RuleContext, file: string, ruleConfig: RuleConfig): readonly Finding[] {
...
function visitDeclarations(ctx: RuleContext, sourceFile: SourceFile, ruleConfig: RuleConfig): readonly Finding[] {
