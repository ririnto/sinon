 ...
 */
export const emptyDirectoryPlaceholdersRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "emptyDirectoryPlaceholders",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.emptyDirectoryPlaceholders;
	...
	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.emptyDirectoryPlaceholders).parameters,
		);
