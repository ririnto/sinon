 ...
 */
export const hookGeneratedMarkerRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "hookGeneratedMarker",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.hookGeneratedMarker;
	...
	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.hookGeneratedMarker).parameters,
		);
		...
		return hooks.flatMap((hook) => {
			...
			return [
				!text.includes(marker)
					? {
							severity: ctx.severityOf(manifest, "hookGeneratedMarker"),
							category: "hookGeneratedMarker",
