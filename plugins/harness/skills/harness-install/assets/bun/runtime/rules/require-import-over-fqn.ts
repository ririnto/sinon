#!/usr/bin/env bun
import { SyntaxKind, createSourceFile, forEachChild, isImportDeclaration, isPropertyAccessExpression, isIdentifier, type Node, type SourceFile } from "typescript@6.0.3";
 ...
 */
export const requireImportOverFqnRule = (ctx: RuleContext): HarnessCheckRule => ({
  applies(manifest: HarnessManifest): boolean {
    const section = manifest.requireImportOverFqn;
  ...
  validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
    return ctx.stackSources(manifest, "typescript").flatMap((file) => {
      ...
      try {
        ...
        return [
          {
            severity: ctx.severityOf(manifest, "requireImportOverFqn"),
      ...
      const visit = (node: Node): void => {
        if (isImportDeclaration(node) && node.importClause && node.importClause.namedBindings && !("name" in node.importClause.namedBindings)) {
