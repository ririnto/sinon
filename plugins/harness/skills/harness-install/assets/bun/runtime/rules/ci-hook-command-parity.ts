 ...
 */
export const ciHookCommandParityRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "ciHookCommandParity",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.ciHookCommandParity;
	...
	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.ciHookCommandParity).parameters,
		);
		const referenceHook =
			typeof parameters.referenceHook === "string"
				? parameters.referenceHook
				: "";
		...
		return ciFiles
			.filter(
				(ciFile) =>
					ctx.isFile(ciFile) && !ctx.read(ciFile).includes(refCommand),
			)
			.flatMap((ciFile) => [
				{
					severity: ctx.severityOf(manifest, "ciHookCommandParity"),
					category: "ciHookCommandParity",
