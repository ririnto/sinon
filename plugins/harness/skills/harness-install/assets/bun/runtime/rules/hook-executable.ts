 ...
 */
export const hookExecutableRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "hookExecutable",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.hookExecutable;
	...
	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.hookExecutable).parameters,
		);
		...
		return hooks.flatMap((hook) => {
			...
			return ctx.isExecutablePath(hook)
				...
				: [
						{
							severity: ctx.severityOf(manifest, "hookExecutable"),
							category: "hookExecutable",
