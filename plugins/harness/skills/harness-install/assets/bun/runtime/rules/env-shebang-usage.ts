 ...
 */
export const envShebangUsageRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "envShebangUsage",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.envShebangUsage;
	...
	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.envShebangUsage).parameters,
		);
		...
		return directories.flatMap((dir) => {
			...
			return warnings.concat(
				files.flatMap((file) => {
					...
					return ctx.firstLine(file).startsWith("#!") &&
						!ctx.firstLine(file).startsWith(expectedPrefix)
						? [
								{
									severity: ctx.severityOf(manifest, "envShebangUsage"),
									category: "envShebangUsage",
