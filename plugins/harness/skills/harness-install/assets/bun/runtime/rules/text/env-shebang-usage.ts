...
export const envShebangUsageRule: HarnessCheckRule = {
    ...
    validate(ctx: RuleContext): readonly Finding[] {
        const parameters = ctx.readJsonObject(ctx.readJsonObject(ctx.manifest.raw.envShebangUsage).parameters);
        const directories = ctx.readStringArray(parameters.directories);
        const expectedPrefix =
            typeof parameters.expectedPrefix === "string" ? parameters.expectedPrefix : "#!/usr/bin/env ";
