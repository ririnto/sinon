 ...
 */
export const implicitLambdaItRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "implicitLambdaIt",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.implicitLambdaIt;
		...
		return (
			Array.isArray(parameters.directories) && parameters.directories.length > 0
		);
	},

	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.implicitLambdaIt).parameters,
		);
		const directories = ctx.readStringArray(parameters.directories);
		const suffix =
			typeof parameters.filenameSuffix === "string"
