...
export const shebangEncodingMarkerRule: HarnessCheckRule = {
  ...
  validate(ctx: RuleContext): readonly Finding[] {
    const parameters = ctx.readJsonObject(
      ctx.readJsonObject(ctx.manifest.raw.shebangEncodingMarker).parameters,
    );
    const markers = ctx.readJsonObject(parameters.markers);
    const messages = ctx.readJsonObject(
      ctx.readJsonObject(ctx.manifest.raw.shebangEncodingMarker).messages,
    );
    ...
    return ctx.stackSources("shebangEncodingMarker").flatMap((file): readonly Finding[] => {
      ...
      const messageTemplate =
        actual.trim().length === 0 ? defaultTemplate : wrongMarkerTemplate;
      return [
        {
          severity,
          category,
