 ...
 */
export const skillFrontmatterRule = (
	ctx: RuleContext,
): HarnessCheckRule => ({
	category: "skillFrontmatter",
	applies(manifest: HarnessManifest): boolean {
		const section = manifest.skillFrontmatter;
	...
	validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
		const parameters = ctx.readJsonObject(
			ctx.readJsonObject(manifest.skillFrontmatter).parameters,
		);
		const rootDirectory =
		...
		if (skillFiles.length === 0) {
			return [
				{
					severity: ctx.severityOf(manifest, "skillFrontmatter"),
					category: "skillFrontmatter",
