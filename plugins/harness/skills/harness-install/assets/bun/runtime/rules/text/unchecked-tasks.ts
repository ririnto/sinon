...
export const uncheckedTasksRule: HarnessCheckRule = {
    ...
    applies(ctx: RuleContext): boolean {
        ...
        return (
            enabled !== false &&
            typeof ctx.readJsonObject((section as Record<string, unknown>).parameters).directory === "string"
        );
    },

    validate(ctx: RuleContext): readonly Finding[] {
        const parameters = ctx.readJsonObject(ctx.readJsonObject(ctx.manifest.raw.uncheckedTasks).parameters);
