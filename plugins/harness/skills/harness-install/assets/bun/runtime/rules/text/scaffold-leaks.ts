...
const compilePortableRegex = (pattern: string): RegExp => {
    ...
    if (inlineFlagMatch?.[1]) {
        return new RegExp(pattern.slice(inlineFlagMatch[0].length), inlineFlagMatch[1]);
    }
...
export const scaffoldLeaksRule: HarnessCheckRule = {
    ...
    applies(ctx: RuleContext): boolean {
        ...
        return (
            enabled !== false &&
            ctx.readStringArray(
                ctx.readJsonObject(ctx.readJsonObject((section as Record<string, unknown>).parameters).scope).bases,
            ).length > 0
        );
    },

    validate(ctx: RuleContext): readonly Finding[] {
        const parameters = ctx.readJsonObject(ctx.readJsonObject(ctx.manifest.raw.scaffoldLeaks).parameters);
        ...
        const patterns: readonly ScaffoldPattern[] = Array.isArray(parameters.patterns)
            ? (parameters.patterns as unknown[])
                  ...
                  .filter(({ patternStr, labelStr }) => patternStr !== "" && labelStr !== "")
                  .map(({ patternStr, labelStr }) => ({ regex: compilePortableRegex(patternStr), label: labelStr }))
            : [];
