...
export const forbidUnsafeSymlinksRule = (ctx: RuleContext): HarnessCheckRule => ({
  ...
  validate(_root: string, manifest: HarnessManifest): readonly Finding[] {
    ...
    const params = ctx.readJsonObject((section as Record<string, unknown>).parameters);
    ...
    const messages = ctx.readJsonObject((section as Record<string, unknown>).messages);
    const scanRootNotAllowedMsg = typeof messages.scanRootNotAllowed === "string" ? messages.scanRootNotAllowed : "symlink scan root is not allowed: {path}";
    const fileNotAllowedMsg = typeof messages.fileNotAllowed === "string" ? messages.fileNotAllowed : "symlink file is not allowed: {path}";
    const pathNotAllowedMsg = typeof messages.pathNotAllowed === "string" ? messages.pathNotAllowed : "symlink path is not allowed: {path}";
    const severity = ctx.severityOf(manifest, "forbidUnsafeSymlinks");
    const findings: Finding[] = [];
    const scanBases = [".claude", "docs", ".github", "AGENTS.md", "CLAUDE.md", "ARCHITECTURE.md"];
    for (const base of scanBases) {
      ...
      for (const file of files) {
        if (ctx.isSymlink(file)) {
          ...
          if (target === null) {
            if (file.split("/").length === 2 && (file === ".claude/AGENTS.md" || file === ".claude/CLAUDE.md")) {
              findings.push({
                severity,
                category: "forbidUnsafeSymlinks",
