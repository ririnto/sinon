...
export const skillFrontmatterRule: HarnessCheckRule = {
    ...
    applies(ctx: RuleContext): boolean {
        ...
        return (
            enabled !== false &&
            ctx.readJsonObject((section as Record<string, unknown>).parameters).rootDirectory !== undefined
        );
    },

    validate(ctx: RuleContext): readonly Finding[] {
        const parameters = ctx.readJsonObject(ctx.readJsonObject(ctx.manifest.raw.skillFrontmatter).parameters);
