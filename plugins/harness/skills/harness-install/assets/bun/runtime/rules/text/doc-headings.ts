...
export const docHeadingsRule: HarnessCheckRule = {
    ...
    applies(ctx: RuleContext): boolean {
        ...
        return (
            enabled !== false &&
            ctx.readStringArray(ctx.readJsonObject((section as Record<string, unknown>).parameters).headings).length > 0
        );
    },

    validate(ctx: RuleContext): readonly Finding[] {
        const parameters = ctx.readJsonObject(ctx.readJsonObject(ctx.manifest.raw.docHeadings).parameters);
        ...
        const filteredFiles = ctx
            .readStringArray(ctx.readJsonObject(ctx.readJsonObject(ctx.manifest.raw[sourceCategory]).parameters).paths)
            .filter((f) => !prefix || (f.startsWith(prefix) && (!suffix || f.endsWith(suffix))));
