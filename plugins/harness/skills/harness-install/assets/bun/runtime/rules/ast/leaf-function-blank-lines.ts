...
export const leafFunctionBlankLinesRule: HarnessCheckRule = {
  ...
  validate(ctx: RuleContext): readonly Finding[] {
    const parameters = ctx.readJsonObject(
      ctx.readJsonObject(ctx.manifest.raw[CATEGORY]).parameters,
    );
    ...
    const maxConsecutiveBlankLines = Number.isFinite(
      ...
      : 1;
    return ctx
      .stackSources(CATEGORY)
      .flatMap((file) => findingsForFile(ctx, file, maxConsecutiveBlankLines));
  },
};
  ...
  const visitNode = (node: Node): readonly Finding[] => {
    switch (node.kind) {
      ...
      case SyntaxKind.Constructor: {
        ...
        const fromHere = funcLike.body && !hasNestedFunctions(funcLike)
