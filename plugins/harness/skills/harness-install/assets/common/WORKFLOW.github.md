...
`WORKFLOW.md` is the source of truth for validation command selection and GitHub publication policy.

| Phase | Action |
| --- | --- |
| Intake | Identify the issue, pull request, local task, or execution plan that owns the change. |
...
| Publish | Use `gh` when a pull request is needed. |
