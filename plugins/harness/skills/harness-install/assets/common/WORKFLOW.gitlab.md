...
The user-facing root session is the sole orchestrator and integration authority.
...
| --- | --- | --- | --- | --- |
| Top-level orchestration and Git integration | `opus` | `gpt-5.6-sol` | `medium` | interactive root session |
...
Record acceptance criteria, difficulty, routing, ownership, base commit, worktree, validation, review, and integration target before dispatch.
...
| Explore | The top-level orchestrator assigns architecture, docs, code, validation, and local Git-context exploration to bounded leaf workers. |
...
1. Intake: identify the user request, local task, plan, or change description.
2. Plan: define verifiable criteria, dependencies, owners, validation, and integration target.
...
11. Integrate: update Git-contained records and apply the approved Git integration method from the root session.

Missing, failed, or contradictory workers block completion.
Worker-branch validation is not integrated validation.

## Git-Contained Records
...
Do not integrate without explicit authority and complete gates.

## Evidence and Completion

Record worker fan-in, validation commands and results, review findings, owner fixes, re-review, manual QA, blockers, and the integrated commit or branch.
Do not report completion while any required evidence or integration result is missing.

Use `autonomous-execution` only when explicitly requested.
Use `issue-mining` for investigation and Git-contained task preparation, not fixes.
