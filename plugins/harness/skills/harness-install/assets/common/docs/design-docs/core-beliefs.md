...
- Repository contracts, validators, and template examples name their purpose and constraints upfront; naming conventions are explicit, not inferred from context.
...
Execution plans live under `docs/exec-plans/` with dates in filenames (`yyyy-MM-dd-<slug>.md`); closed plans remain in `docs/exec-plans/` so history and decision rationale stay auditable.
...
Deferred documentation and technical debt compound silently; by the time a refactor sprint happens, agents have already wasted cycles on stale guidance, and the project is harder to navigate. Small, incremental care keeps the contracts and docs aligned with reality and reduces surprise failures during onboarding.
