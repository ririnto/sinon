...
| --- | --- | --- | --- | --- |
| Availability (HTTP 2xx/3xx) | 99.9% | 28 days | Page on 2x burn at 1h | {{owner}} |
| p95 request latency | < 500 ms | 28 days | Page on 2x burn at 1h | {{owner}} |
| Event delivery success | 99.9% | 28 days | Page on 2x burn at 1h | {{owner}} |
| Daily backup completion | 100% | 1 day | Page on 1 missed run | {{owner}} |
...
- **Traces**: propagated via `traceparent` header and exported via OTLP to `{{otlp-endpoint}}`.
