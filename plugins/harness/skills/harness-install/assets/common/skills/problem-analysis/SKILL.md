---
name: problem-analysis
description: >-
  Investigate reported or suspected repository problems and recommend whether to record them as a task or change description. Use for specific problems, duplicates, related reports, likely causes, or open-ended improvement candidates.
---

# Problem Analysis

Investigate and report. Do not implement a fix or create a record unless the user asks.

## First Safe Checks

1. Identify a named problem or an open-ended request for candidates.
2. Define a bounded evidence surface: records, code, tests, logs, validation output, and docs.
3. Decide whether external evidence is needed for duplicates, upstream behavior, dependencies, releases, or user reports.

## Procedure

1. Read the named record and linked artifacts, or survey the bounded repository surface for candidates.
2. Search existing task and change records, review records, and recent reports for duplicates or related work.
3. Inspect relevant code, tests, logs, validation output, and docs.
4. Check external sources only when they affect the conclusion.
5. Report cause, impact, evidence, affected files, and duplicate or related records.
6. Record findings only with explicit user authority, using the repository task or change-description templates, a plan, or a local review note.

## Decisions

- Keep repository exploration read-only unless the user asks to record findings.
- Name missing evidence and owner decisions as blockers.
- Use native delegation only when it materially improves bounded investigation.
- Follow the repository documented workflow or contribution guidance for local records when one exists.

## Output Contract

Return:

- `summary`: status and recommendation.
- `duplicates`: matching or related records.
- `cause`: likely source and affected files.
- `evidence`: internal and external sources checked.
- `record`: created or updated record, or reason not recorded.
- `blockers`: missing evidence or owner decisions.

When a record is requested, prepare `title`, `problem statement`, `evidence`, `affected files or systems`, `expected behavior`, `current behavior`, `duplicate or related records`, and `validation or reproduction notes`.
