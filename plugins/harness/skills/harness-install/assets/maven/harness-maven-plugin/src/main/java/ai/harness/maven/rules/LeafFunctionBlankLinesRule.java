 ...
 */
public enum LeafFunctionBlankLinesRule implements HarnessCheckRule {
    INSTANCE;

    @Override
    public String category() {
        return "leafFunctionBlankLines";
    }
    ...
    public Collection<Finding> validate(Path root, JsonNode manifest) throws MojoExecutionException {
        ...
        try {
            ...
            return HarnessCheckHelper.stackSources(manifest, CATEGORY).stream()
            ...
            if ("\n".equals(text) || "\r\n".equals(text)) {
                ...
                if (maxConsecutiveBlankLines + 1 < lastNewlineCount.get()) {
                    token.getRange().map(r -> r.begin.line).ifPresent(line -> {
                        if (0 < line) {
                            findings.add(new Finding(severity, CATEGORY, root.relativize(file) + ":" + (line - 1) + ": too many blank lines in leaf function"));
                        }
                    });
                }
