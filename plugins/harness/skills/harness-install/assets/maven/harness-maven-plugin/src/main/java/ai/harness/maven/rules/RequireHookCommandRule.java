...
public enum RequireHookCommandRule implements HarnessCheckRule {
    ...
    public Collection<Finding> validate(Path root, JsonNode manifest) throws MojoExecutionException {
        ...
        final String prePushText = HarnessCheckHelper.readFile(root, prePush);
        final String declaredCmd = extractHookCommand(prePushText);
        final List<Finding> prePushIssues = declaredCmd.isEmpty()
    ...
    private List<Finding> validatePreCommitHook(Path root, String severity, Path preCommit) throws MojoExecutionException {
        if (HarnessCheckHelper.isSafeRegularFile(root, preCommit)) {
            final String preCommitText = HarnessCheckHelper.readFile(root, preCommit);
