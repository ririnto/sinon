 ...
 */
public enum ImportOverFqnRule implements HarnessCheckRule {
    INSTANCE;

    @Override
    public String category() {
        return "importOverFqn";
    }
    ...
    private List<Finding> validateImportOverFqn(Path root, Path file, String severity) {
        try {
            ...
            return Stream.concat(
                    ...
                    .map(candidate -> new Finding(severity, CATEGORY, root.relativize(file) + ":" + candidate.line() + ": fully qualified name `" + candidate.qualifiedName() + "` used inline; add an import and use the simple name"))
                    .toList();
        } catch (IOException e) {
