...
def read_rule_config(ctx: RuleContext) -> RuleConfig:
    ...
    allowed_patterns = (
        params.get("allowedPatterns", []) if isinstance(params, dict) else []
    )
    python_dunder_exemptions = (
        params.get("pythonDunderExemptions", True) if isinstance(params, dict) else True
    )
    return RuleConfig(
...
class LeadingUnderscoreRule(HarnessCheckRule):
    ...
    def validate_file(
        self, path: Path, ctx: RuleContext, severity: str, rule_config: RuleConfig
    ) -> tuple[Finding, ...]:
        """Validate one Python file."""
        basename = path.stem
        basename_findings = (
            (self.finding(ctx, path, basename, 1, severity),)
            if rule_config.is_forbidden(basename)
            else ()
        )
        tree, error = parse_python(path)
        if error is not None or tree is None:
            return basename_findings + (
                Finding(
                    severity,
                    self.category,
    ...
    def finding(
        self, ctx: RuleContext, path: Path, name: str, line: int, severity: str
    ) -> Finding:
...
class DeclarationVisitor(cst.CSTVisitor):
    ...
    def __init__(
        self, ctx: RuleContext, path: Path, severity: str, rule_config: RuleConfig
    ) -> None:
    ...
    def record(self, name: str, node: cst.CSTNode) -> None:
        ...
        if self.rule_config.is_forbidden(name):
            ...
            self.findings.append(
                LeadingUnderscoreRule().finding(
                    self.ctx, self.path, name, pos.start.line, self.severity
                )
            )
