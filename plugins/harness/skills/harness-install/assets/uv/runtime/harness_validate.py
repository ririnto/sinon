...
def is_executable(path: Path) -> bool:
    ...
    if path.is_symlink():
        ...
        if target is None:
            ...
            path = target
            try:
                result = bool(path.stat().st_mode & stat.S_IXUSR)
            except OSError:
                result = False
    else:
        try:
...
def safe_file_or_walk(base: Path) -> tuple[Path, ...]:
    ...
    if base.is_symlink() and allowed_root_contract_target(base) is None:
        ...
        result = (base,) if is_safe_file(base) else safe_walk(base)
    return result
...
def severity_for(manifest: dict, category: str) -> str:
    ...
    if isinstance(section, dict):
        ...
        result = value if value in ("ERROR", "WARN", "INFO") else "ERROR"
    else:
        result = "ERROR"
    return result
                    ...
                    if not isinstance(messages, dict):
                        ...
                        try:
                            ...
                            if not is_safe_directory(dir_path):
                                result = (Finding(severity_for(manifest, self.category), self.category, missing_dir_msg),)
                            else:
            ...
            if not isinstance(params, dict):
                ...
                if not isinstance(allowed_cmds, dict) or not isinstance(messages, dict):
                    ...
                    if not isinstance(stack_commands, list):
                        result = (Finding("ERROR", self.category, f"validation command for stack '{STACK}' missing from manifest"),)
                    else:
                    ...
                    if not isinstance(directory, str):
                        ...
                        if not is_safe_directory(dir_path):
                            ...
                            try:
                                ...
                                result = (Finding("ERROR", self.category, f"invalid uncheckedTaskPattern regex: {unchecked_pattern_str}"),)
                            else:
                                import fnmatch
                ...
                if not isinstance(allowed_pairs, list):
                    ...
                    if not isinstance(messages, dict):
                        ...
                        for base in scan_bases:
                            ...
                            elif is_safe_directory(base_path):
