    ...
    def validate(self, root: Path, manifest: dict) -> tuple[Finding, ...]:
        ...
        return tuple(
            Finding(
                severity_for(manifest, self.category),
                self.category,
                messages.get("default", "executable script should use /usr/bin/env shebang: {file}").format(file=relative(path)),
            )
            for directory in directories
            if isinstance(directory, str)
    ...
    def validate(self, root: Path, manifest: dict) -> tuple[Finding, ...]:
        ...
        return tuple(
            Finding(
                severity_for(manifest, self.category),
                self.category,
                messages.get("default", "completed plan has unchecked tasks: {file}").format(file=relative(path)),
            )
