...
/**
 * Rule that requires companion objects to be positioned at the top of the class.
 */
object RequireCompanionObjectPositionRule : HarnessCheckRule {
	/**
	 * PSI detection result for a misplaced companion object.
	 */
	@Serializable
	data class Result(
		val file: String,
	...
	override fun applies(manifest: JsonObject): Boolean {
		val category = "requireCompanionObjectPosition"
	...
	override fun validate(manifest: JsonObject, root: Path, psiResults: HarnessPsiResults?): Collection<Finding> {
		...
		return if (catObj == null || parametersObj == null || messagesObj == null || sourceRootsPerStack == null || extensionsPerStack == null) {
			...
			val results = psiResults?.companionPositions ?: emptyList()
				...
				if (!dir.exists()) {
					...
					files.filter { file ->
						...
						results.filter { it.file == file.name }.map { hit ->
							Finding(HarnessCheck.severityOf(manifest, category), category, HarnessCheck.stringFrom(messagesObj, "default").takeIf { it.isNotEmpty() } ?: "${file.relativeTo(root)}:${hit.line}: companion object in class `${hit.className}` must be positioned at the top")
						}
					}
				}
			}
