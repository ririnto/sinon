 ...
 */
class UncheckedCastSuppressionKtlintRule : KtlintRule(
    ...
    companion object {
        private val FORBIDDEN_SUPPRESSIONS: EditorConfigProperty<String> =
            EditorConfigProperty(
                type = PropertyType("ktlint_unchecked_cast_suppression_forbidden", "Comma separated forbidden suppression tokens", PropertyType.PropertyValueParser.IDENTITY_VALUE_PARSER),
                defaultValue = "UNCHECKED_CAST",
            )

        private val ALLOWED_SUPPRESSIONS: EditorConfigProperty<String> =
            EditorConfigProperty(
                type = PropertyType("ktlint_unchecked_cast_suppression_allowed", "Comma separated allowed suppression tokens", PropertyType.PropertyValueParser.IDENTITY_VALUE_PARSER),
                defaultValue = "",
            )
    }
    ...
    override fun beforeFirstNode(editorConfig: EditorConfig) {
        forbiddenTokens = editorConfig[FORBIDDEN_SUPPRESSIONS].split(",").map { it.trim() }.filter { it.isNotEmpty() }.toSet()
        allowedTokens = editorConfig[ALLOWED_SUPPRESSIONS].split(",").map { it.trim() }.filter { it.isNotEmpty() }.toSet()
    }
                    ...
                    override fun visitAnnotationEntry(annotation: KtAnnotationEntry) {
                        ...
                        if (annotation.shortName?.asString() == "Suppress") {
                            if (extractSuppressTokens(annotation).intersect(forbiddenTokens - allowedTokens).isNotEmpty()) {
                                emit(annotation.textOffset, "avoid suppression of forbidden tokens (`${annotation.text}`); refactor to type-safe cast or explicit handling", false)
                            }
                        }
                    }
