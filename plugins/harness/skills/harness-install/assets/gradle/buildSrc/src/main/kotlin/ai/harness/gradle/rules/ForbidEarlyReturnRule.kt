				...
				if (!dir.exists()) {
					...
					files.filter { file ->
						...
						results.filter { it.file == file.name }.map { hit ->
							Finding(HarnessCheck.severityOf(manifest, category), category, HarnessCheck.stringFrom(messagesObj, "default").takeIf { it.isNotEmpty() } ?: "${file.relativeTo(root)}:${hit.line}: function `${hit.function}` has an early/mid return; restructure with single exit")
						}
					}
				}
			}
