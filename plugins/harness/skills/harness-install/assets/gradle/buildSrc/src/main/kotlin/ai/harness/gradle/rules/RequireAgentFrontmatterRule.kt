    ...
    override fun validate(
        ...
        return if (catObj == null || parametersObj == null || messagesObj == null) {
            ...
            if (!dirPath.isDirectory()) {
                ...
                if (files.isEmpty()) {
                    val msg =
                        HarnessCheck.stringFrom(messagesObj, "missingAgent").takeIf { it.isNotEmpty() }
                            ?: ".claude/agents must contain at least one .md agent"
                    listOf(Finding(severity, category, msg))
                } else {
                    files.flatMap { file ->
                        ...
                        if (!text.startsWith("---")) {
                            val msg =
                                HarnessCheck.stringFrom(messagesObj, "missingFrontmatter").takeIf { it.isNotEmpty() }
                            ...
                            listOfNotNull(
                                if (!"""(?m)^name:\s*[-a-z0-9]+\s*$""".toRegex().containsMatchIn(text)) {
                                    val msg =
                                        HarnessCheck.stringFrom(messagesObj, "missingName").takeIf { it.isNotEmpty() }
