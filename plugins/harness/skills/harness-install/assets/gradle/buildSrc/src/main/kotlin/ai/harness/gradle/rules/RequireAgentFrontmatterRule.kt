    ...
    override fun validate(
        ...
        return if (catObj == null || parametersObj == null || messagesObj == null) {
            ...
            if (!dirPath.isDirectory()) {
                ...
                /**
                 * Directory read may fail due to permission or I/O issues; return empty on silent fallback.
                 */
                val files =
                    try {
                        dirPath.listDirectoryEntries().filter { entry -> entry.isRegularFile() && entry.extension == "md" }
                    } catch (e: Exception) {
                ...
                if (files.isEmpty()) {
                    val msg =
                        HarnessCheck.stringFrom(messagesObj, "missingAgent").takeIf { message -> message.isNotEmpty() }
                            ?: ".claude/agents must contain at least one .md agent"
                    listOf(Finding(severity, category, msg))
                } else {
                    files.flatMap { file ->
                        ...
                        if (!text.startsWith("---")) {
                            val msg =
                                HarnessCheck.stringFrom(messagesObj, "missingFrontmatter").takeIf { message -> message.isNotEmpty() }
                            ...
                            listOfNotNull(
                                if (!"""(?m)^name:\s*[-a-z0-9]+\s*$""".toRegex().containsMatchIn(text)) {
                                    val msg =
                                        HarnessCheck.stringFrom(messagesObj, "missingName").takeIf { message -> message.isNotEmpty() }
