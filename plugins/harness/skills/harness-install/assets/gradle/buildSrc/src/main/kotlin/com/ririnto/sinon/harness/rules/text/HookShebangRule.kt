            ...
            addAll(
                ctx.manifest.stringArray(category, "hooks")
                    .filter { hookPath ->
                        ...
                        when {
                            !hook.isRegularFile() -> false
                            else -> (hook.readLines().firstOrNull() ?: "") != JsonAccess.stringFromObject(parametersObj, "expectedShebang")
                        }
                    }.map { hookPath ->
                        Finding(
                            ...
                            ctx.manifest.stringValue(category, "default").takeIf { message ->
                                message.isNotEmpty()
                            } ?: "$hookPath must start with ${JsonAccess.stringFromObject(parametersObj, "expectedShebang")}",
                        )
                    }
            )
        }
