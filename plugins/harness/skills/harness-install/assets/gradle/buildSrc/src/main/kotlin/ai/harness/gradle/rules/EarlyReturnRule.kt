 ...
 */
object EarlyReturnRule : HarnessCheckRule() {
            ...
            ktFile.accept(
                object : KtTreeVisitorVoid() {
                    override fun visitNamedFunction(function: KtNamedFunction) {
                        ...
                        val statements = function.bodyBlockExpression?.statements ?: return
                        statements
                            .filter { statement -> statement::class.simpleName == "KtReturnExpression" }
                            .filter { statement -> statement !== statements.last() }
                            .forEach { statement ->
                                add(
                                    PsiFinding(
                                        rule = category,
                                        file = PsiRuleSupport.relativeFilePath(file, root),
                                        line = PsiRuleSupport.lineOf(ktFile, statement.node?.startOffset),
