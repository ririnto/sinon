...
class ImportOverFqnKtlintRule(
     ...
     */
    companion object {
        private const val CATEGORY: String = "importOverFqn"
    }
        ...
        (node.psi as? KtFile)?.let { ktFile ->
            ...
            fun addFqnFinding(nameParts: List<String>, element: PsiElement) {
                if (isPackageQualifiedName(nameParts) && nameParts.lastOrNull() !in importedNames) {
                    emit(element.textOffset, message(mapOf("name" to nameParts.joinToString("."))), false)
                }
            }
     ...
     *
     * A package-qualified name has at least 3 parts separated by dots, with the first two parts
     * being lowercase (package parts) and the last part being uppercase (class or type name).
     */
