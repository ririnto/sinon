 ...
 */
class PublicDeclarationDocCommentKtlintRule : KtlintRule(
    ruleId = RuleId("code:public-declaration-doc-comment"),
) {
    companion object {
        private val visibility = setOf("public", "protected", "internal")
        private val allowedDeclarationKinds = setOf("class", "function", "property")
        private val exemptOverrideDeclarations = true
        private val exemptLocalDeclarations = true
    }
                ...
                object : KtTreeVisitorVoid() {
                    override fun visitClass(klass: KtClass) {
                        ...
                        if (shouldCheck(klass, "class") && klass.docComment == null) {
                            emit(klass.textOffset, "public declaration `${klass.name ?: "unknown"}` is missing a documentation comment", false)
                        }
                    }
