...
/**
 * Rule that forbids unstructured logging via print and println.
 */
object ForbidUnstructuredLoggingRule : HarnessCheckRule {
	/**
	 * PSI detection result for unstructured logging.
	 */
	@Serializable
	data class Result(
		val file: String,
	...
	override fun applies(manifest: JsonObject): Boolean {
		val category = "forbidUnstructuredLogging"
	...
	override fun validate(manifest: JsonObject, root: Path, psiResults: HarnessPsiResults?): Collection<Finding> {
		...
		return if (catObj == null || parametersObj == null || messagesObj == null || sourceRootsPerStack == null || extensionsPerStack == null) {
			...
			val results = psiResults?.unstructuredLoggings ?: emptyList()
				...
				if (!dir.exists()) {
					...
					files.filter { file ->
						...
						results.filter { it.file == file.name }.map { hit ->
							Finding(HarnessCheck.severityOf(manifest, category), category, HarnessCheck.stringFrom(messagesObj, "default").takeIf { it.isNotEmpty() } ?: "${file.relativeTo(root)}:${hit.line}: use a logger instead of print or println")
						}
					}
				}
			}
