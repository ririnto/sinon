...
description: >-
  Install target-owned repository harness assets from the plugin asset package: `AGENTS.md` contract, `CLAUDE.md` pointer, `ARCHITECTURE.md`, docs structure, project agents, project skills, structured repository templates, language-matched validators, CI files, and Git hook templates.
...
4. Identify the active CI host.
   - Use the target repository policy to select GitHub, GitLab, both, or neither; remove unused CI files only as an explicit post-install change.
...
| Target mirrors to GitHub and GitLab | Pass `--ci-host both` to write both CI files. The installed `pre-push` final check command must match both scripts. |
...
- Report project readiness only after product specs, architecture decisions, acceptance criteria, and generated artifacts contain target truth.
