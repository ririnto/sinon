...
4. Identify the active CI host. Use the target repository policy to select GitHub, GitLab, both, or neither; remove unused CI files only as an explicit post-install change.
...
- `ci`: which CI files were rendered (`.github/workflows/harness.yml`, `.gitlab-ci.yml`, both, or none) and the selected CI host.
