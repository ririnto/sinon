...
The operating system delivers bytes to the application in arbitrary chunks regardless of how the sender called `write()`.
Two independent `write()` calls on the sender side can arrive as one merged buffer or split across multiple `read()` callbacks on the receiver.
A framing decoder reassembles these byte fragments into complete, meaningful message boundaries before business handlers see them.
