...
- Check branch sync before pushing when an upstream exists.
  ...
  - Resolve behind or diverged state before publishing.
...
- Publish only the intended committed state: never publish from a tree with uncommitted or blocking untracked changes.
...
Behind or diverged state must be resolved before publishing.
