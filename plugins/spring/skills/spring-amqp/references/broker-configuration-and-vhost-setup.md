...
- When specifying `spring.rabbitmq.addresses` with an `amqps://` URI, SSL is enabled automatically.
  The separate `spring.rabbitmq.ssl.enabled` property is not needed in that case.
