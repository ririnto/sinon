...
```java
/**
 * Holds one component instance per issuer so delegating repositories resolve the tenant that the current request targets.
 */
public final class TenantPerIssuerComponentRegistry {
    ...
    /**
     * Returns the component registered for the current request issuer.
     */
...
class TenantRegisteredClientRepository implements RegisteredClientRepository {
    ...
    /**
     * Delegates the client lookup to the repository registered for the current issuer.
     */
    @Override
...
class TenantOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    /**
     * Delegates the authorization persistence to the service registered for the current issuer.
     */
    @Override
...
class TenantJWKSource implements JWKSource<SecurityContext> {
    ...
    /**
     * Delegates key selection to the JWK source registered for the current issuer.
     */
    @Override
