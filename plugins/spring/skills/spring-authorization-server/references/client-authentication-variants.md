...
```java
/**
 * Converts token-endpoint requests that carry the application's custom client-authentication data into a client authentication token.
 */
public class CustomClientAuthenticationConverter implements AuthenticationConverter {
    /**
     * Returns the client authentication for the request, or {@code null} when the request does not carry the custom credentials.
     */
    @Override
