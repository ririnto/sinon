...
- Registered clients, authorization state, or consent must survive restarts or be shared across cluster nodes, and the persistence choice is modeled in [`jpa-persistence.md`](jpa-persistence.md) or [`redis-persistence.md`](redis-persistence.md)
...
For multiple issuers, do not configure an issuer globally and route requests and components through issuer-scoped configuration in [multitenancy.md](multitenancy.md).
