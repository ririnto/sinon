...
Use the port that actually serves Actuator.
If `management.server.port` is separate from the main server port, point the probe at that management port or expose probe paths on the main port explicitly.
