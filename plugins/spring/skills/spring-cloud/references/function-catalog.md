...
- Function registry `size()` and `names()` are now consistent after concurrent lookups (5.0.3).
  - If upgrading from an earlier version, verify that any custom registry listeners still behave correctly.
