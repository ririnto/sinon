...
description: "Open this when the blocker is deciding whether an alert needs regression coverage or which lifecycle behaviors the sibling testing path should protect."
---
...
Actual `promtool test rules` file authoring belongs to [`../../alert-rule-testing/SKILL.md`](../../alert-rule-testing/SKILL.md). Keep baseline alert anatomy in [`../SKILL.md`](../SKILL.md).

## When This Alert Likely Needs Regression Coverage

Hand off to the sibling testing skill when any of these are true:
...
- Use the sibling testing skill when the blocker becomes fixture design, `input_series`, `eval_time`, `alert_rule_test`, or runnable `promtool test rules` commands.
