...
description: >-
  Use this skill when the user asks to decide whether changes are ready for one commit, draft a Conventional Commit message from the real diff, prepare hosted review text, or preserve repository repository templates while integrateing a change.
---
...
Use `feat`, `fix`, `docs`, `test`, `refactor`, `perf`, `build`, `ci`, or `chore`, and prefer `feat` or `fix` whenever integrateed behavior changes. If the diff mixes unrelated feature work, broad cleanup, or separate test concerns, split the commit before drafting final text.
