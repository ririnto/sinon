...
test("workflow documents carry the complete lifecycle and selected-host split", async () => {
  ...
  const skill = await Bun.file(
    path.join(
      root,
      "plugins/workspace-workflow/skills/change-description/SKILL.md"
    )
  ).text();
  ...
  expect(
    await Bun.file(
      path.join(
        root,
        "plugins/workspace-workflow/skills/change-description/references/github.md"
      )
    ).exists()
  ).toBe(true);
  expect(
    await Bun.file(
      path.join(
        root,
        "plugins/workspace-workflow/skills/change-description/references/gitlab.md"
      )
    ).exists()
  ).toBe(true);
